	...
	<span>
		...
		<div class="todo-list ex1" v-if="tasks">
			<div class="todo-item" v-for="(task, index) in tasks" :key="task.id">
				...
				<span v-if="task.status == 2">
					<span><del>{{task.name | strippedContent}}</del></span>
				</span>
				<span v-else>
					<span>
					    ...
					    <!-- <span contenteditable @input="handleInput" v-model="editname" v-on:keyup.13.prevent="editdata(task.id)">{{task.name}}</span> -->
					</span>
				</span>
...
<script type="text/javascript">
    ...
    methods: {
        getData() {
            axios.get('api/show')
                .then((response) => {
                    this.tasks = response.data.showtasks
                })
        },
        counttask() {
